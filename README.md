...
（菜单按钮颜色校验与默认值）、浏览器注册（插槽注册/卸载级联/共享 store/
面板交互/AI 批准与完成提醒卡片）。另有 `verify-e2e.ts` / `verify-notifications.ts`
两个真实浏览器端到端脚本。

## 验收证据（真实浏览器，本机 dsh web 127.0.0.1:3080）

- 单元测试：**69/69 通过**（`pnpm exec vitest run packages/client/ui-leisure-games`）。
- `verify-e2e.ts`：**33/33 通过** —— 侧边栏按钮位于「新会话」与「工作区」之间；
  ...
  **棋盘格为正方形（boardFrame 宽高差 < 2px）且棋子居中落在横竖线交界处**；
  ...
  首点安全、右键插旗正常；设置面板分区齐全；**外观颜色自定义（颜色持久化并
  应用到侧边栏按钮与面板主色）**；退出游戏返回聊天界面。
